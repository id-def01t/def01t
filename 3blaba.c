  ...
  while (1) {                    //Переберём все элементы
    if ((!ElemCurr)||(ElemCurr->ch=='\t')||(ElemCurr->ch==' ')) { //Если встретился разделитель или конец списка
      if (ElemCurr==Elem0) {     //Если элемент первый, то
        ...
        if (flag) {         //Если число не удовлетворяет критерию, то удалить его (изначально флаг 0 => ничего не потеряем)
    ...
    } else {                      //если встретилось число то проверяем его 
      if ((ElemCurr->ch<'0')||(ElemCurr->ch>'9')) flag=1; //Любой другой символ кроме цифры нарушает критерий отбора.
